...
ANTHROPIC_STREAM_CODE_GENERATION_TOOL_EVENTS_FIXTURE = JSON.parse(File.read(File.expand_path("../fixtures/anthropic_stream/code_generation_tool_events.json", __dir__)), symbolize_names: true)
ANTHROPIC_STREAM_CODE_GENERATION_EXPECTED_FIXTURE = JSON.parse(File.read(File.expand_path("../fixtures/anthropic_stream/code_generation_expected.json", __dir__)), symbolize_names: true)
...
class StreamTest < Test
  ...
  test "test chunk mapping code generation tool stream" do
    mapper = LlmGateway::Adapters::Anthropic::StreamMapper.new

    accumulator = StreamAccumulator.new
    ANTHROPIC_STREAM_CODE_GENERATION_TOOL_EVENTS_FIXTURE.each do |chunk|
      accumulator.push(mapper.map(chunk))
    end

    assert_equal ANTHROPIC_STREAM_CODE_GENERATION_EXPECTED_FIXTURE, accumulator.result
  end
end

...
class HandoffStreamLiveTest < Test
  ...
  def run_handoff_stream_for(provider:, model:, adapter:)
    ...
    prompt = <<~PROMPT
      You are receiving recorded final outputs from previous general streaming test runs.
      The records include text streaming and tool-use streaming results. Read the JSON and answer in one
      short sentence. Include all of these facts as Arabic numerals where numeric: the number of recorded
      outputs, the tool-call sum 15 + 27, the multiplication result 42 * 17, and the sum 453 + 434.
    ...
    assert_includes text, "714"
    assert_includes text, "887"
  end

  def self.define_handoff_stream_test_for(provider:, model:)
    test "handoff_stream__#{provider}_#{model}" do
  ...
  def load_recorded_outputs
    skip "Missing fixture directory at #{FIXTURE_DIR}. Run stream_test live tests first." unless Dir.exist?(FIXTURE_DIR)
    ...
    Dir.glob(File.join(FIXTURE_DIR, "*.json")).sort.map do |path|
      ...
      skip "No recorded outputs in #{FIXTURE_DIR}. Run stream_test live tests first." if records.empty?
    end
  end
end
